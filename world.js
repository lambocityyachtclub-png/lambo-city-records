...
export default {
  init(scene) {
    var groundMat = new THREE.MeshStandardMaterial({ color: 0x3a2810, roughness: 1 });
    ...
    [{z:8},{z:-16},{z:-40}].forEach(function(vp) {
      ...
      for (var row = 0; row < 2; row++) {
        for (var col = 0; col < 4; col++) {
          var win = new THREE.Mesh(
            ...
            new THREE.MeshStandardMaterial({ color: 0xffcc66, emissive: 0xffaa33, emissiveIntensity: 0.9, transparent: true, opacity: 0.9 })
          );
      ...
      var pool = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x00aacc, emissive: 0x00aacc, emissiveIntensity: 0.8, transparent: true, opacity: 0.85 })
      );

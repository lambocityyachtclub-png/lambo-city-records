...
export default {
  init(scene) {
    ...
    // Dock ends at the gate / roundabout transition.
const base = new THREE.Mesh(new THREE.BoxGeometry(14, 0.4, 72), woodMat);
base.position.set(0, Y, -22);
    scene.add(base);
    for (let z = -58; z < 15; z += 2) {
      const p = new THREE.Mesh(new THREE.BoxGeometry(13.5, 0.15, 1.2), plankMat);
      p.position.set(0, Y+0.28, z);
      scene.add(p);
    }
    [-6,6].forEach(x => {
      for (let z = -58; z < 15; z += 8) {
    ...
    [-6.2,6.2].forEach(x => {
      const r = new THREE.Mesh(new THREE.BoxGeometry(0.12,0.6,72), railMat);
r.position.set(x, Y+0.6, -22);
      scene.add(r);
    });

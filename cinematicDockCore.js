...
export default {
  init(scene) {
    ...
    socialData.forEach(function(d) {
      ...
      var back = new THREE.Mesh(new THREE.BoxGeometry(14,6,0.4), new THREE.MeshStandardMaterial({color:0x050505}));
      group.add(back);
      var border = new THREE.Mesh(
        new THREE.BoxGeometry(14.5,6.5,0.2),
        new THREE.MeshStandardMaterial({color:d.accent,emissive:d.accent,emissiveIntensity:0.5})
      );
      border.position.z = -0.2;
      group.add(border);
      var bar = new THREE.Mesh(
        new THREE.BoxGeometry(12,1.5,0.5),
        new THREE.MeshStandardMaterial({color:d.accent,emissive:d.accent,emissiveIntensity:0.4})
      );

...
export default {
  init(scene) {
    ...
    var neon = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x00ffff, emissive: 0x00ffff, emissiveIntensity: 3 })
    );
    ...
    [-1,1].forEach(function(side) {
      for (var i = 0; i < 4; i++) {
        var win = new THREE.Mesh(
          ...
          new THREE.MeshStandardMaterial({ color: 0x88ccff, transparent: true, opacity: 0.7, emissive: 0x224466, emissiveIntensity: 0.5 })
        );
    ...
    yacht.add(goldLight);
    // hull bottom at Y = -1.5, sits in water
    yacht.position.set(38, -1.2, -28);

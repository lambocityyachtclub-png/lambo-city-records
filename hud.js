export default {
  ...
  _buildHUD() {
    ...
    this._add(hud,`<div id="music-hud" style="position:absolute;top:16px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.75);border:1px solid rgba(255,215,0,0.35);border-radius:24px;padding:7px 18px;display:flex;align-items:center;gap:10px;pointer-events:all;box-shadow:0 0 20px rgba(153,0,255,0.25);white-space:nowrap;"><div style="color:#9900ff;font-size:16px;">♪</div><div><div style="color:#ffd700;font-size:9px;letter-spacing:2px;">NOW PLAYING</div><div style="color:white;font-size:11px;">LAMBO CITY — THE TAKEOVER</div></div><div style="width:70px;height:3px;background:#333;border-radius:2px;overflow:hidden;"><div id="music-progress" style="width:0%;height:100%;background:linear-gradient(90deg,#9900ff,#ff00aa);border-radius:2px;"></div></div><button id="music-btn" style="background:linear-gradient(135deg,#9900ff,#ff00aa);border:none;border-radius:50%;width:28px;height:28px;color:white;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;box-shadow:0 0 10px rgba(153,0,255,0.5);">▶</button></div>`);
  ...
  update(delta,context) {
    ...
    var yachtX=35,yachtZ=-25;
    var dx=player.position.x-yachtX;
    var dz=player.position.z-yachtZ;
    var dist=Math.sqrt(dx*dx+dz*dz);
    var maxDist=60;
    var progress=Math.min(100,Math.max(0,((maxDist-dist)/maxDist)*100));
  ...
  _showBoardingPass() {
    ...
    card.innerHTML = `<div style="color:#ffd700;font-size:10px;letter-spacing:4px;margin-bottom:8px;">⚓ YOU MADE IT ⚓</div><div style="font-size:22px;font-weight:bold;letter-spacing:3px;margin-bottom:12px;">BOARDING PASS</div><div style="border:1px solid rgba(255,215,0,0.3);border-radius:8px;padding:14px 20px;margin:12px 0;background:rgba(255,215,0,0.05);"><div style="color:#00ffff;font-size:12px;letter-spacing:2px;">LAMBO CITY YACHT CLUB</div><div style="color:#aaa;font-size:10px;margin-top:4px;">VIP MEMBER — EXCLUSIVE ACCESS</div><div style="font-size:30px;letter-spacing:6px;margin:10px 0;color:#ffd700;font-weight:bold;text-shadow:0 0 20px rgba(255,215,0,0.5);">LC-2024</div><div style="color:#666;font-size:9px;letter-spacing:1px;">CITIZEN OF LAMBO CITY • LONG BEACH, CA</div></div><div style="color:#aaa;font-size:11px;line-height:1.7;margin-bottom:16px;">You've unlocked the Yacht Club.<br>Exclusive events await inside.</div><button onclick="this.parentElement.remove()" style="background:linear-gradient(90deg,#9900ff,#ff00aa);border:none;border-radius:20px;color:white;padding:10px 28px;font-size:12px;letter-spacing:2px;cursor:pointer;">⚓ BOARD THE YACHT →</button>`;

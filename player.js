...
export default {
  init(scene) {
    ...
    // Normal gameplay floor.
    this._floorY = 1.3;

    // When not null, the elevator temporarily controls Y.
  ...
  clearElevatorControl() {
    ...
    if (player) {
      player.position.y = this._floorY ?? 1.3;
    }
  },
  ...
  update(delta, context) {
    ...
    const speed =
      (sprint ? this.sprintSpeed : this.speed) * delta;
    ...
    if (input.joystick?.active) {
      if (Math.abs(input.joystick.x) > 0.08) {
        dx = input.joystick.x;
        moving = true;
      }

      if (Math.abs(input.joystick.y) > 0.08) {
        dz = input.joystick.y;
        moving = true;
      }
    }
    ...
    // ----------------------------------------------------------
    //
    // Elevator control takes priority.
    //
    // This prevents the old 1.3 Y reset from fighting the
    // elevator while it is moving between floors.

    if (this._elevatorY !== null) {
      ...
      if (moving) {
        ...
        this.armL.rotation.x *= 0.82;
        this.armR.rotation.x *= 0.82;

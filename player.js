...
export default {
  init(scene) {
    ...
    var chain = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0xffd700, emissive: 0xffd700, emissiveIntensity: 0.4, metalness: 1, roughness: 0.2 })
    );
  ...
  update(delta, context) {
    ...
    if (input.joystick?.active) {
      ...
      if (Math.abs(jx) > 0.1 || Math.abs(jy) > 0.1) { dx = jx; dz = jy; moving = true; }
    }
    ...
    if (moving) {
      ...
      player.position.y = 1.3 + Math.sin(bobTime * 1.5) * 0.04;
    }

...
export default {
  init(scene) {
    ...
    for (var i = 0; i < 5; i++) {
      ...
      var coneMat = new THREE.MeshBasicMaterial({color:colors[i],transparent:true,opacity:0.05,side:THREE.DoubleSide});
    ...
    var neonMat = new THREE.MeshStandardMaterial({color:0x9900ff,emissive:0x9900ff,emissiveIntensity:2});
    [-12,-6,0,6,12].forEach(function(x) {
      var strip = new THREE.Mesh(new THREE.BoxGeometry(0.15,0.05,18),neonMat);
      strip.position.set(x,1.55,-70);
      scene.add(strip);
    });
  },
  update(delta) {
